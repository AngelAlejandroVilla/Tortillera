...
export default {
  ...
  methods: {
    editDeliver() {
      let Post = {
        UserId: this.Casher.UserId,
        CompanyId: this.$store.state.Company,
        ProfileId: this.Casher.ProfileId,
        Status: this.Casher.Status,
    ...
    getCasher(id) {
      axios
        ...
        .then((result) => {
          this.Casher = result.data[0];
          console.log(this.Casher);
          this.nickname = this.Casher.Nickname;
          this.password = this.Casher.Password;
        });
    },
  },
};
